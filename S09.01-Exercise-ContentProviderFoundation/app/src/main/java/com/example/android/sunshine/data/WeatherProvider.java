...
//      TODO (9) Handle queries on both the weather and weather with date URI
        ...
        switch (sUriMatcher.match(uri)) {
            case CODE_WEATHER:
                result = db.query(WeatherContract.WeatherEntry.TABLE_NAME, projection, selection, selectionArgs, null, null, sortOrder);
                break;
            case CODE_WEATHER_WITH_DATE:

                String query = WeatherContract.WeatherEntry.COLUMN_DATE + "=?";
                String normalizedUtcDateString = uri.getLastPathSegment();
                String[] date = new String[]{normalizedUtcDateString};
                result = db.query(WeatherContract.WeatherEntry.TABLE_NAME, projection, query, date, null, null, sortOrder);
                break;
            default: throw new RuntimeException("Student, you need to implement the delete method!");
        }
