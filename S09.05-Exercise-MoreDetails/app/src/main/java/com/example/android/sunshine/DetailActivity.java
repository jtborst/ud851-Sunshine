...
//  TODO (20) Create a constant int to identify our loader used in DetailActivity
    ...
    private static final int LOADER_DETAIL_ID = 27;
...
//  TODO (10) Remove the mWeatherDisplay TextView declaration
    ...
    TextView mWeatherDate, mWeatherDescription, mWeatherHighTemp, mWeatherLowTemp, mWeatherHumidity, mWeatherWind, mWeatherPressure;
...
//      TODO (13) Find each of the TextViews by ID
        mWeatherDate = (TextView) findViewById(R.id.view_date);
        mWeatherDescription = (TextView) findViewById(R.id.view_description);
        mWeatherHighTemp = (TextView) findViewById(R.id.view_high_temp);
        mWeatherLowTemp = (TextView) findViewById(R.id.view_low_temp);
        mWeatherHumidity = (TextView) findViewById(R.id.view_humidity);
        mWeatherWind = (TextView) findViewById(R.id.view_wind);
        mWeatherPressure = (TextView) findViewById(R.id.view_pressure);
...
//      TODO (14) Remove the code that checks for extra text
        ...
        if (intentThatStartedThisActivity != null) {
            ...
            if (uri == null) {
                throw new NullPointerException("please supply a uri");
            }
        }
...
//      TODO (33) Store a forecast summary in mForecastSummary
        ...
        long dateLong = data.getLong(data.getColumnIndex(WeatherContract.WeatherEntry.COLUMN_DATE));

        mWeatherDate.setText(SunshineDateUtils.getFriendlyDateString(getBaseContext(), dateLong, true));

        int weatherId = data.getInt(data.getColumnIndex(WeatherContract.WeatherEntry.COLUMN_WEATHER_ID));

        mWeatherDescription.setText(SunshineWeatherUtils.getStringForWeatherCondition(this, weatherId));

        mWeatherHighTemp.setText(SunshineWeatherUtils.formatTemperature(this, data.getDouble(data.getColumnIndex(WeatherContract.WeatherEntry.COLUMN_MAX_TEMP))));
        mWeatherLowTemp.setText(SunshineWeatherUtils.formatTemperature(this, data.getDouble(data.getColumnIndex(WeatherContract.WeatherEntry.COLUMN_MIN_TEMP))));

        mWeatherHumidity.setText(Float.toString(data.getFloat(data.getColumnIndex(WeatherContract.WeatherEntry.COLUMN_HUMIDITY))));

        mWeatherWind.setText(SunshineWeatherUtils.getFormattedWind(this, data.getFloat(data.getColumnIndex(WeatherContract.WeatherEntry.COLUMN_WIND_SPEED)),
                data.getFloat(data.getColumnIndex(WeatherContract.WeatherEntry.COLUMN_DEGREES))));

        mWeatherPressure.setText(Float.toString(data.getFloat(data.getColumnIndex(WeatherContract.WeatherEntry.COLUMN_PRESSURE))));
    }
